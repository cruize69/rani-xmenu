...
import { MAX_CART_METADATA_CHUNKS } from "./sanitize.js";
...
export async function getOrCreateOrderForSession(session, paymentIntent = null, shouldNotify = true) {
  ...
  let cartItems = [];
  try {
    let cartJson = session.metadata?.cart;
    if (!cartJson) {
      const chunks = [];
      for (let i = 0; i < MAX_CART_METADATA_CHUNKS; i++) {
        const chunk = session.metadata?.[`cart_${i}`];
        if (chunk) chunks.push(chunk);
        else break;
      }
      cartJson = chunks.join("");
    }
