...
import { sendOrderEmail, sendCustomerReceiptEmail, sendOrderSMS, sendEmail } from "./notifications.js";
...
export async function getOrCreateOrderForSession(session, paymentIntent = null, shouldNotify = true) {
  ...
  if (shouldNotify) {
    ...
    if (order.status !== "scheduled") jobs.push(notifyPrintQueue(order.id));
