  ...
  jobs.push(sendEmail({
    ...
    html: `
      <div style="font-family:sans-serif;padding:20px;background:#fff3e0;border:2px solid #d9482c;border-radius:10px;max-width:520px;">
        ...
        <p style="margin:0 0 6px;"><strong>Contact:</strong> ${contactLine || "unknown"}</p>
        <p style="margin:0 0 6px;"><strong>Cart:</strong> ${cartLine}</p>
        <p style="margin:0 0 6px;"><strong>Order mode:</strong> ${lead.orderMode ?? "unknown"}</p>
        <p style="margin:0 0 6px;"><strong>Error source:</strong> ${source}</p>
        <p style="margin:0 0 12px;"><strong>Details:</strong> ${safeMessage}</p>
        ${lead.phone ? `<a href="tel:${lead.phone}" style="display:inline-block;background:#d9482c;color:#fff;padding:10px 20px;border-radius:8px;text-decoration:none;font-weight:700;">Call ${lead.phone}</a>` : ""}
      </div>`,
  }));
  if (process.env.RESTAURANT_PHONE) {
    jobs.push(sendSMS(process.env.RESTAURANT_PHONE, summary.slice(0, 1550)));
  }
  ...
  jobs.push(sendEmail({
    ...
    html: `
      <div style="font-family:sans-serif;padding:20px;background:#fdecea;border:3px solid #b91c1c;border-radius:10px;max-width:520px;">
        ...
        <p style="margin:0 0 6px;">The customer <strong>${name}</strong> was charged <strong>${amount}</strong> — Stripe confirms it — but something broke building the order record. It will not appear on any kitchen screen.</p>
        <p style="margin:0 0 6px;"><strong>Contact:</strong> ${contactLine}</p>
        <p style="margin:0 0 6px;"><strong>Stripe session:</strong> ${session?.id}</p>
        <p style="margin:0 0 12px;"><strong>Error:</strong> ${(error?.message ?? "unknown").slice(0, 400)}</p>
        ${phone ? `<a href="tel:${phone}" style="display:inline-block;background:#b91c1c;color:#fff;padding:10px 20px;border-radius:8px;text-decoration:none;font-weight:700;">Call ${phone}</a>` : ""}
      </div>`,
  }));
  if (process.env.RESTAURANT_PHONE) {
    jobs.push(sendSMS(process.env.RESTAURANT_PHONE, summary.slice(0, 1550)));
