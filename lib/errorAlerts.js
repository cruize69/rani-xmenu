...
// For email SUBJECT lines and SMS bodies. escapeHtml is the wrong tool for
// both: a subject is a header (CR/LF = header injection) and an SMS is
// plain text, where escaping would just show staff literal "&lt;" noise.
// What actually matters in both is that attacker-supplied text can't add
// new lines and fake extra fields — a staff SMS reading like an official
// multi-line alert is the whole spoofing risk here (fake urgency, phishing
// instructions), and it's reachable anonymously via api/report-error.js.
...
export async function reportCheckoutError({ draftId, source, message, context = {} }) {
  ...
  // The identifiable path below gets a staff SMS/email — real-time, but
  // per-contact-cooldown throttled, so it's not meant as a durable record.
  // The unidentifiable path (an anonymous visitor, no phone/email on file
  // yet — e.g. a sign-in error hit before entering any contact info) used
  // to end at the console.error above: real in Vercel's function logs, but
  // nobody was ever going to go read them. A real customer (Jeremy, Aug 19
  // 2026 — Clerk sign-in failed silently, no visibility until he mentioned
  // it in person) hit exactly this gap. Sentry now gets every reported
  // error either way, so this class of complaint no longer needs a
  // customer to volunteer it.
  captureServerError(new Error(`[checkout-error:${source}] ${safeMessage}`), { draftId, identifiable, source, ...context });
  ...
  const summary = `⚠️ Checkout error — customer stuck!\n${oneLine(contactLine)}\nCart: ${oneLine(cartLine, 200)}\nError (${oneLine(source, 60)}): ${oneLine(safeMessage, 300)}\nCall them now to finish the order.`;

  const jobs = [];
  jobs.push(sendEmail({
    to: STAFF_EMAILS,
    subject: `⚠️ Customer stuck at checkout — call ${oneLine(lead.phone || lead.email, 80)}`,
    html: `
...
export async function reportPaidOrderBuildFailed({ session, error }) {
  ...
  const summary = `🚨 URGENT: ${oneLine(name, 80)} was CHARGED ${oneLine(amount, 20)} but no order was created!\n${oneLine(contactLine)}\nStripe session: ${oneLine(session?.id, 80)}\nCall them immediately — the food isn't in the kitchen queue.`;

  const jobs = [];
  jobs.push(sendEmail({
    to: STAFF_EMAILS,
    subject: `🚨 URGENT: Charged ${oneLine(amount, 20)} but order failed — ${oneLine(name, 80)}`,
    html: `
