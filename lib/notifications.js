...
const FROM = process.env.RESEND_FROM_EMAIL || "orders@ranimahal.food";
...
async function sendEmail({ to, subject, html }) {
  ...
  try {
    ...
    if (!res.ok) {
      ...
      // If custom domain is not yet DNS-verified in Resend dashboard, retry with onboarding fallback
      if ((errText.includes("not verified") || errText.includes("validation_error") || errText.includes("domain")) && FROM !== "onboarding@resend.dev") {
        console.warn("Retrying email via onboarding@resend.dev fallback due to unverified custom domain...");
        ...
        if (!res.ok) {
          console.error("Resend fallback error:", await res.text());
        }
      }
    }
  } catch (err) {
