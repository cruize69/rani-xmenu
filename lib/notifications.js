...
export function secondOrderTouch1SmsBody({ isMember }) {
  ...
  return `Rani Mahal: How was your first order? ${tail} https://ranimahal.cc/order Reply STOP to opt out.`;
}
...
export function secondOrderTouch2EmailHtml({ customerName, isMember, link }) {
  ...
  return marketingEmailShell(`
    ...
    </div>`, isMember ? "Your 5% member discount is ready whenever you are." : "Here's 10% off your next order.");
}

export function secondOrderTouch2SmsBody({ isMember, link }) {
  return isMember
    ? `Rani Mahal: Come back for round two? Your 5% member discount is ready whenever you are: https://ranimahal.cc/order Reply STOP to opt out.`
