...
// The old writereview?placeid=... link 404'd for real customers — that
// exact placeid is also what ranimahal.cc's own "Leave a Review" button
// still uses, so this was broken everywhere, not just here (likely a
// stale/reclaimed listing ID). I couldn't browse Google Maps from this
// environment to pull the current correct Place ID, so rather than guess
// another one that might also be wrong, this uses Google's documented
// Maps Search URL API (search by exact name+address) — it reliably lands
// on the correct listing every time, it just costs the customer one extra
// tap (open listing → Reviews → Write a review) instead of landing
// straight in the review composer. For a true one-tap link, grab the
// "Ask for reviews" short link from Google Business Profile directly and
// swap it in here — that's the one guaranteed-correct source for it.
const GOOGLE_WRITE_REVIEW_URL = "https://www.google.com/maps/search/?api=1&query=" +
  encodeURIComponent("Rani Mahal Fine Indian Cuisine, 327 Mamaroneck Ave, Mamaroneck, NY 10543");
