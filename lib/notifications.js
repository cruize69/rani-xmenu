...
/**
 * Staff-facing new-order alert — sent to the restaurant (ranimahal327@gmail.com).
 */
export async function sendOrderEmail(order) {
  const { RESTAURANT_EMAIL } = process.env;
  const defaultStaff = "ranimahal327@gmail.com";
  const recipients = (RESTAURANT_EMAIL ?? defaultStaff).split(",").map(s => s.trim()).filter(Boolean);

  const isDelivery = order.orderMode === "delivery";
  const shortId = order.id.slice(-6).toUpperCase();
  const townName = order.deliveryAddress?.city?.trim() || "Westchester";
  const modeLabel = isDelivery ? `🚗 DELIVERY [${townName}]` : "🛍️ PICKUP";
