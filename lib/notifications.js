...
import { isCateringOrder } from "./menu.js";
import { formatTime } from "./hours.js";
import { formatPhoneNumber } from "./printer.js";

// ranimahal.food was deleted as a verified sending domain in Resend once
// ranimahal.cc was added and verified — sending FROM the old domain would
// now bounce, since Resend requires FROM's domain to be one it has SPF/
// DKIM verified for. Independent of which URL customers browse (BASE_URL,
// see ranimahal.cc/order elsewhere in this file) — this is specifically
// about which domain is authorized to send mail on this business's behalf.
const FROM = "orders@ranimahal.cc";
...
//
// Mobile-first specifically (most opens happen on a phone, not desktop):
// - `preview` renders as a hidden preheader — the snippet Gmail/Apple
//   Mail/Outlook mobile show in the inbox LIST, before the email is even
//   opened. Without one, clients fall back to grabbing the first visible
//   text on the page (here, always "Rani Mahal" from the header), so
//   every single one of these emails showed the identical, useless
//   preview snippet in a crowded inbox. Padded with invisible characters
//   so a short preview doesn't leak header/footer text as filler.
// - format-detection:telephone=no stops iOS Mail from auto-styling the
//   footer's phone number as a blue underlined link, which looks like a
//   theming bug against this dark design.
// - Single ~480px-max fluid column throughout (no multi-column layout
//   anywhere) — already the layout that works without a media query, but
//   called out here since it's the actual reason no @media block is
//   needed the way the transactional order-receipt shell has one.
function marketingEmailShell(innerHtml, preview = "") {
  const previewPadding = "&#8203;&nbsp;".repeat(Math.max(0, 20 - Math.floor(preview.length / 6)));
  return `
<!DOCTYPE html>
<html lang="en">
<head>
...
<body style="margin:0;padding:0;background-color:#080706;font-family:'Inter',-apple-system,BlinkMacSystemFont,sans-serif;-webkit-font-smoothing:antialiased;">
  ${preview ? `<div style="display:none;max-height:0;overflow:hidden;opacity:0;mso-hide:all;">${escapeHtml(preview)}${previewPadding}</div>` : ""}
        ...
        <table role="presentation" width="100%" cellspacing="0" cellpadding="0" border="0" style="max-width:480px;">
          <tr>
            <td style="text-align:center;padding:4px 0 22px;">
              <img src="https://ranimahal.cc/logo/apsara-logo-256.png" alt="Rani Mahal" width="56" height="56" style="display:block;margin:0 auto 10px;border:0;outline:none;border-radius:50%;box-shadow:0 0 18px rgba(232,168,46,0.4);" />
          ...
          <tr><td style="padding:0 4px;">${innerHtml}</td></tr>
          <tr>
            <td style="text-align:center;padding:24px 0 0;">
              <p style="font-family:'Inter',sans-serif;font-size:12px;color:#B8A995;margin:0 0 12px;line-height:1.8;">
                <a href="https://ranimahal.cc/order" target="_blank" style="color:#E8A82E;text-decoration:none;font-weight:600;">Online Menu</a> &nbsp;·&nbsp;
                <a href="https://ranimahal.cc/order/rewards" target="_blank" style="color:#E8A82E;text-decoration:none;font-weight:600;">Rani Royal Club</a>
              </p>
...
// Full-width, block-level CTA — every marketing email's single primary
// action button. Previously `display:inline-block` sized to the label
// text, which on a phone means tapping a button roughly as wide as
// "Redeem 10% Off" — comfortable height (padding already cleared 44px)
// but a narrow, precise target. table role="presentation" wrapper (not a
// bare block div) because Outlook's Word rendering engine ignores CSS
// width/padding on plain elements but respects it on table cells.
function ctaButton(href, label) {
  return `
    <table role="presentation" width="100%" cellspacing="0" cellpadding="0" border="0" style="margin-top:4px;">
      <tr>
        <td align="center" style="background-color:#E8A82E;border-radius:24px;">
          <a href="${href}" style="display:block;width:100%;box-sizing:border-box;padding:15px 20px;color:#080706;font-family:'Inter',sans-serif;font-weight:700;font-size:15px;text-decoration:none;text-align:center;">${label}</a>
        </td>
      </tr>
    </table>`;
}
 ...
 */
function orderEmailShell({ eyebrow, heading, bodyBefore = "", bodyAfter = "", order, showContact = false }) {
        ...
        <table role="presentation" class="email-container" width="100%" cellspacing="0" cellpadding="0" border="0" style="max-width:520px;background-color:#12100e;border:1px solid rgba(232,168,46,0.35);border-radius:18px;overflow:hidden;box-shadow:0 16px 48px rgba(0,0,0,0.85);">
          ...
          <tr>
            <td style="background-color:#0d0c0a;padding:32px 20px 22px;text-align:center;border-bottom:2px solid #E8A82E;">
              <table role="presentation" width="100%" cellspacing="0" cellpadding="0" border="0">
                <tr>
                  <td align="center">
                    <img src="https://ranimahal.cc/logo/apsara-logo-256.png" alt="Rani Mahal Logo" width="64" height="64" style="display:block;margin:0 auto 10px;border:0;outline:none;border-radius:50%;box-shadow:0 0 18px rgba(232,168,46,0.45);" />
          ...
          <tr>
            <td class="mobile-padding" style="padding:26px 20px;">
              ...
              ${order.scheduledFor ? `
              <table role="presentation" width="100%" cellspacing="0" cellpadding="0" border="0" style="background-color:rgba(232,168,46,0.12);border:1px solid rgba(232,168,46,0.4);border-radius:12px;margin-bottom:18px;">
                <tr>
                  <td style="padding:12px 16px;font-family:'Inter',sans-serif;font-size:14px;font-weight:700;color:#E8A82E;">
                    🕐 Requested ${isDelivery ? "delivery" : "pickup"} time: ${formatTime(order.scheduledFor.time)}
                  </td>
                </tr>
              </table>` : ""}
              ${showContact && (order.customerPhone || order.customerEmail) ? `
              <table role="presentation" width="100%" cellspacing="0" cellpadding="0" border="0" style="background-color:#1a1612;border:1px solid rgba(232,168,46,0.25);border-radius:12px;margin-bottom:18px;">
                <tr>
                  <td style="padding:12px 16px;">
                    <p style="font-family:'Inter',sans-serif;font-size:11px;font-weight:700;letter-spacing:0.1em;text-transform:uppercase;color:#E8A82E;margin:0 0 6px;">Customer Contact</p>
                    ${order.customerPhone ? `<a href="tel:${order.customerPhone}" style="display:block;font-family:'Inter',sans-serif;font-size:16px;font-weight:700;color:#FAF6EF;text-decoration:none;margin-bottom:2px;">📞 ${formatPhoneNumber(order.customerPhone)}</a>` : ""}
                    ${order.customerEmail ? `<a href="mailto:${order.customerEmail}" style="display:block;font-family:'Inter',sans-serif;font-size:13px;color:#B8A995;text-decoration:none;">✉ ${escapeHtml(order.customerEmail)}</a>` : ""}
                  </td>
                </tr>
              </table>` : ""}
              ${bodyBefore}
              ...
              <table role="presentation" width="100%" cellspacing="0" cellpadding="0" border="0" style="background-color:#1a1612;border-radius:14px;padding:18px;margin-bottom:24px;border:1px solid rgba(250,246,239,0.06);">
                ...
                <tr>
                  <td style="font-family:'Inter',sans-serif;font-size:13px;color:#B8A995;padding:5px 0;">Menu Subtotal</td>
                  <td align="right" style="font-family:'Inter',sans-serif;font-size:13px;color:#FAF6EF;padding:5px 0;">$${(order.originalSubtotal ?? (order.subtotal + order.discountAmount)).toFixed(2)}</td>
                </tr>
                <tr>
                  <td style="font-family:'Inter',sans-serif;font-size:13px;color:#10B981;font-weight:600;padding:5px 0;">${escapeHtml(order.discountLabel || "Promotional Discount")}</td>
                  <td align="right" style="font-family:'Inter',sans-serif;font-size:13px;color:#10B981;font-weight:700;padding:5px 0;">-$${order.discountAmount.toFixed(2)}</td>
                </tr>
                <tr>
                  <td style="font-family:'Inter',sans-serif;font-size:13px;color:#B8A995;padding:5px 0;">Discounted Subtotal</td>
...
function reorderVoucherBlock(order) {
  ...
  const link = `https://ranimahal.cc/order?reorder=${order.reorderToken}`;
  // Catering is always flat-rate (see lib/menu.js's isCateringOrder / the
  // CATERING_ITEM_IDS discount exclusion in create-checkout.js) — this
  // voucher can never discount another catering order, only a future
  // regular menu order, so a catering receipt needs to say that plainly
  // rather than imply "anytime" covers a repeat catering booking too.
  const eyebrow = isCateringOrder(order) ? "For Your Next Regular Order" : "Your Next Order";
  const body = isCateringOrder(order)
    ? `Take <strong style="color:#E8A82E;">10% off</strong> your next regular menu order (pickup or delivery) anytime in the next 14 days.`
    : `Take <strong style="color:#E8A82E;">10% off</strong> anytime in the next 14 days.`;
  return `
              <table role="presentation" width="100%" cellspacing="0" cellpadding="0" border="0" style="background-color:#1a1612;border:1px dashed rgba(232,168,46,0.5);border-radius:14px;margin-bottom:24px;">
                <tr>
                  <td style="padding:18px;text-align:center;">
                    <p style="font-family:'Inter',sans-serif;font-size:11px;font-weight:700;letter-spacing:0.14em;text-transform:uppercase;color:#E8A82E;margin:0 0 6px;">${eyebrow}</p>
                    <p style="font-family:'Fraunces',Georgia,serif;font-size:16px;color:#FAF6EF;margin:0 0 12px;">${body}</p>
                    <table role="presentation" width="100%" cellspacing="0" cellpadding="0" border="0">
                      <tr>
...
const STATUS_EMAIL = {
    ...
    note: order.orderMode === "delivery"
      ...
      : "Your order is packed fresh and waiting for you — easiest parking and entrance is at 320-322 Phillips Park Rd. Come on in!",
  }),
};
...
export async function sendOrderSMS(order) {
  ...
  // A scheduled order's relevant time is the requested slot, not when it
  // was placed — showing createdAt here (as this used to, unconditionally)
  // reads as "order came in at 2pm" when what staff actually need to know
  // is "this is due at 5pm," which they'd otherwise have no way to see
  // outside of Order Manager itself.
  //
  // The immediate-order branch had two real bugs, both confirmed against
  // live production SMS the owner actually received (Aug 20 2026):
  // toLocaleTimeString() with no `timeZone` renders in whatever zone the
  // server process runs in — Vercel functions run in UTC — so a 5:04pm ET
  // order was texted as a bare "9:04 PM" with no indication it was even a
  // clock time, let alone which zone. Both are fixed here: explicit
  // America/New_York, and a label so it reads the same shape as the
  // scheduled-order line above it instead of a mystery number.
  const timeLine = order.scheduledFor
    ? `⏰ FOR ${formatTime(order.scheduledFor.time)}`
    : `🕐 Placed ${new Date(order.createdAt).toLocaleTimeString("en-US", { hour: "numeric", minute: "2-digit", timeZone: "America/New_York" })}`;
  // Staff need to know pickup vs. delivery at a glance — how an order gets
  // staged/handed off differs, and this line was previously the only piece
  // of order-shape info missing from the alert entirely.
  const modeLine = order.orderMode === "delivery" ? "🚗 DELIVERY" : "🏠 PICKUP";
  const discountLine = order.discountAmount > 0
    ? `\n🎉 Discount: -$${order.discountAmount.toFixed(2)} (${order.discountType === "welcome" ? "10% Welcome" : order.discountType === "member" ? "5% Member" : "Voucher"})`
    : "";
...
export function reviewNudgeEmailHtml({ customerName }) {
  ...
  return marketingEmailShell(`
    <div style="background:#12100e;border:1px solid rgba(232,168,46,0.35);border-radius:18px;padding:32px 24px;text-align:center;color:#FAF6EF;">
      ...
      ${ctaButton(GOOGLE_WRITE_REVIEW_URL, "⭐ Leave a Google Review")}
    </div>`, `How was dinner, ${customerName && customerName !== "Guest" ? customerName : "there"}? A quick review helps us more than you'd think.`);
}
...
export function winBackEmailHtml({ customerName, link, isMember, favoriteDish }) {
  const name = escapeHtml(customerName && customerName !== "Guest" ? customerName : "there");
  const headline = favoriteDish
    ? `Craving your ${escapeHtml(favoriteDish)} again, ${name}?`
    : `We miss cooking for you, ${name}`;
  return marketingEmailShell(`
    <div style="background:#12100e;border:1px solid rgba(232,168,46,0.35);border-radius:18px;padding:32px 24px;text-align:center;color:#FAF6EF;">
      ...
      <h1 style="font-family:Georgia,serif;font-size:24px;margin:0 0 14px;">${headline}</h1>
      ...
      ${ctaButton(link, "Redeem 10% Off →")}
      <p style="font-size:11px;color:#8A7560;margin:20px 0 0;">Valid for 14 days · one-time use</p>
    </div>`, "It's been a while — here's 10% off to bring you back.");
}

export function winBackSmsBody({ link, isMember, favoriteDish }) {
  const tail = isMember ? "" : " Sign in for 5% off every order after.";
  const hook = favoriteDish ? `Craving your ${favoriteDish} again?` : "We miss you!";
  return `Rani Mahal: ${hook} 10% off your next order: ${link}${tail} Reply STOP to opt out.`;
}
...
export function secondOrderTouch1EmailHtml({ customerName, isMember, isCatering }) {
  const name = escapeHtml(customerName && customerName !== "Guest" ? customerName : "there");
  ...
  const heading = isCatering ? `How did your event go, ${name}?` : `How was your first order, ${name}?`;
  const body = isCatering
    ? `Hope it was a hit! The same kitchen that catered your event is here for a regular night in too — full menu, same tandoor. ${memberLine}`
    : memberLine;
  return marketingEmailShell(`
    <div style="background:#12100e;border:1px solid rgba(232,168,46,0.35);border-radius:18px;padding:32px 24px;text-align:center;color:#FAF6EF;">
      <h1 style="font-family:Georgia,serif;font-size:22px;margin:0 0 14px;">${heading}</h1>
      <p style="font-size:14px;color:#B8A995;line-height:1.6;margin:0 0 24px;">${body}</p>
      ${ctaButton("https://ranimahal.cc/order", "See the Menu →")}
    </div>`, body);
}

export function secondOrderTouch1SmsBody({ isMember, isCatering }) {
  const tail = isMember ? "Your 5% member discount is already on for next time." : "Sign in next time for 5% off every order.";
  const opener = isCatering ? "How did your event go? Try us for a regular night in too —" : "How was your first order?";
  return `Rani Mahal: ${opener} ${tail} https://ranimahal.cc/order Reply STOP to opt out.`;
}

export function secondOrderTouch2EmailHtml({ customerName, isMember, link, isCatering }) {
  const name = escapeHtml(customerName && customerName !== "Guest" ? customerName : "there");
  const heading = isCatering ? `Try us for yourself, ${name}?` : `Come back for round two, ${name}?`;
  const body = isMember
    ? "Your 5% member discount is ready whenever you are — no code needed."
    : "Here's 10% off your next order. Sign in when you use it and 5% off every order after kicks in automatically.";
  return marketingEmailShell(`
    <div style="background:#12100e;border:1px solid rgba(232,168,46,0.35);border-radius:18px;padding:32px 24px;text-align:center;color:#FAF6EF;">
      <h1 style="font-family:Georgia,serif;font-size:22px;margin:0 0 14px;">${heading}</h1>
      <p style="font-size:14px;color:#B8A995;line-height:1.6;margin:0 0 24px;">${isCatering ? `Loved having you cater with us — ${body.charAt(0).toLowerCase()}${body.slice(1)}` : body}</p>
      ${ctaButton(isMember ? "https://ranimahal.cc/order" : link, isMember ? "See the Menu →" : "Redeem 10% Off →")}
      ${isMember ? "" : `<p style="font-size:11px;color:#8A7560;margin:20px 0 0;">Valid for 14 days · one-time use</p>`}
    </div>`, body);
}

export function secondOrderTouch2SmsBody({ isMember, link, isCatering }) {
  const opener = isCatering ? "Loved catering with you — try us for yourself?" : "Come back for round two?";
  return isMember
    ? `Rani Mahal: ${opener} Your 5% member discount is ready whenever you are: https://ranimahal.cc/order Reply STOP to opt out.`
    : `Rani Mahal: ${opener} 10% off your next order: ${link} Sign in for 5% off every order after. Reply STOP to opt out.`;
}
...
export function winBackTouch2EmailHtml({ customerName, link, isMember, favoriteDish }) {
  const name = escapeHtml(customerName && customerName !== "Guest" ? customerName : "there");
  const headline = favoriteDish
    ? `Still craving your ${escapeHtml(favoriteDish)}, ${name}?`
    : `Still there, ${name}?`;
  return marketingEmailShell(`
    <div style="background:#12100e;border:1px solid rgba(232,168,46,0.35);border-radius:18px;padding:32px 24px;text-align:center;color:#FAF6EF;">
      ...
      ${ctaButton(link, "Redeem 10% Off →")}
      <p style="font-size:11px;color:#8A7560;margin:20px 0 0;">Valid for 14 days · one-time use</p>
    </div>`, "Last call — your 10% offer expires soon.");
}

export function winBackTouch2SmsBody({ link, isMember, favoriteDish }) {
  const tail = isMember ? "" : " Sign in for 5% off every order after.";
  const hook = favoriteDish ? `Still craving your ${favoriteDish}?` : "Still there?";
  // Plain hyphen, not an em-dash — the only non-GSM-7 character in this
  // string. GSM-7 vs UCS-2 isn't just a display nuance: a single em-dash
  // forces the WHOLE message into UCS-2 encoding, which drops the segment
  // limit from 160 to 70 characters. At this message's length that meant
  // 3 billed segments instead of 1 — silently 3x the Twilio cost with no
  // visible sign anything was wrong, since the string itself is well
  // under 160 chars.
  return `Rani Mahal: ${hook} 10% off expires soon: ${link}${tail} Reply STOP to opt out.`;
}
...
export function newsletterWelcomeEmailHtml({ link }) {
  return marketingEmailShell(`
    <div style="background:#12100e;border:1px solid rgba(232,168,46,0.35);border-radius:18px;padding:32px 24px;text-align:center;color:#FAF6EF;">
      ...
      ${ctaButton(link, "Redeem 10% Off →")}
      <p style="font-size:11px;color:#8A7560;margin:20px 0 0;">Valid for 14 days · one-time use</p>
    </div>`, "You're on the list — here's 10% off to start with.");
}
...
export function neverOrderedNudgeEmailHtml({ link }) {
  return marketingEmailShell(`
    <div style="background:#12100e;border:1px solid rgba(232,168,46,0.35);border-radius:18px;padding:32px 24px;text-align:center;color:#FAF6EF;">
      ...
      ${ctaButton(link, "Order & Save 10% →")}
      <p style="font-size:11px;color:#8A7560;margin:20px 0 0;">Valid for 14 days · one-time use</p>
    </div>`, "Your 10% welcome offer is still sitting there unused.");
}
...
export function newsletterDigestEmailHtml({ dishName, dishDesc }) {
  return marketingEmailShell(`
    <div style="background:#12100e;border:1px solid rgba(232,168,46,0.35);border-radius:18px;padding:32px 24px;text-align:center;color:#FAF6EF;">
      ...
      ${ctaButton("https://ranimahal.cc/order", "See the Full Menu →")}
    </div>`, `This month: ${dishName} — ${dishDesc}`);
}

// ── Cultural calendar (api/cron/cultural-calendar.js) ────────────────
// A lead-time reminder ahead of a named holiday/feast — deliberately no
// discount attached (same "stay warm" logic as the newsletter digest):
// this is a "plan your feast with us" nudge for an occasion people are
// already spending on, not a discount-driven win-back.
export function culturalEventEmailHtml({ customerName, eventName, blurb, daysAway }) {
  const name = escapeHtml(customerName && customerName !== "Guest" ? customerName : "there");
  return marketingEmailShell(`
    <div style="background:#12100e;border:1px solid rgba(232,168,46,0.35);border-radius:18px;padding:32px 24px;text-align:center;color:#FAF6EF;">
      <p style="font-size:11px;letter-spacing:0.2em;text-transform:uppercase;color:#E8A82E;font-weight:700;margin:0 0 12px;">${escapeHtml(eventName)} is coming up</p>
      <h1 style="font-family:Georgia,serif;font-size:22px;margin:0 0 14px;">Planning your ${escapeHtml(eventName)} feast, ${name}?</h1>
      <p style="font-size:14px;color:#B8A995;line-height:1.6;margin:0 0 24px;">${escapeHtml(blurb)} ${eventName} is only ${daysAway} days away — order ahead so it's ready exactly when you want it.</p>
      ${ctaButton("https://ranimahal.cc/order", "Order for " + eventName + " →")}
    </div>`, `${eventName} is ${daysAway} days away — order ahead.`);
}

export function culturalEventSmsBody({ eventName, daysAway }) {
  return `Rani Mahal: ${eventName} is ${daysAway} days away. Order ahead for your feast: https://ranimahal.cc/order Reply STOP to opt out.`;
}
...
export function cateringCrossSellEmailHtml({ customerName }) {
  ...
  return marketingEmailShell(`
    <div style="background:#12100e;border:1px solid rgba(232,168,46,0.35);border-radius:18px;padding:32px 24px;text-align:center;color:#FAF6EF;">
      ...
      ${ctaButton("https://ranimahal.cc/order/catering", "Get a Catering Quote →")}
    </div>`, "We cater office lunches, parties, and events too.");
}

export function cateringCrossSellSmsBody() {
  return `Rani Mahal: Planning something bigger? We cater office lunches, parties & events too: https://ranimahal.cc/order/catering Reply STOP to opt out.`;
}
...
export function abandonedLeadEmailHtml({ cartLine, link }) {
  return marketingEmailShell(`
    <div style="background:#12100e;border:1px solid rgba(232,168,46,0.35);border-radius:18px;padding:32px 24px;text-align:center;color:#FAF6EF;">
      ...
      ${ctaButton(link, "Finish Checkout →")}
    </div>`, `Your order (${cartLine}) is still saved.`);
}

export function abandonedDraftTouch1EmailHtml({ cartLine, link, isCatering }) {
  const heading = isCatering ? "Still planning your event?" : "You left something delicious behind";
  const lead = isCatering ? "Your package selection is still saved:" : "Your order is still saved:";
  return marketingEmailShell(`
    <div style="background:#12100e;border:1px solid rgba(232,168,46,0.35);border-radius:18px;padding:32px 24px;text-align:center;color:#FAF6EF;">
      <h1 style="font-family:Georgia,serif;font-size:22px;margin:0 0 14px;">${heading}</h1>
      <p style="font-size:14px;color:#B8A995;line-height:1.6;margin:0 0 20px;">${lead}</p>
      <p style="font-size:14px;color:#FAF6EF;font-weight:600;margin:0 0 24px;">${escapeHtml(cartLine)}</p>
      ${ctaButton(link, isCatering ? "Finish Booking →" : "Finish Checkout →")}
    </div>`, `${isCatering ? "Your catering selection" : "Your order"} (${cartLine}) is still saved.`);
}

export function abandonedDraftTouch2EmailHtml({ cartLine, link, isCatering }) {
  // Catering is always flat-rate — no "10% off" claim here, unlike the
  // a-la-carte version below (see sendDraftTouch2 in lib/abandonedCart.js
  // for why: the checkout itself would never actually honor that discount
  // on a catering item).
  if (isCatering) {
    return marketingEmailShell(`
      <div style="background:#12100e;border:1px solid rgba(232,168,46,0.35);border-radius:18px;padding:32px 24px;text-align:center;color:#FAF6EF;">
        <h1 style="font-family:Georgia,serif;font-size:22px;margin:0 0 14px;">Still time to book your event</h1>
        <p style="font-size:14px;color:#B8A995;line-height:1.6;margin:0 0 20px;">Still waiting for you:</p>
        <p style="font-size:14px;color:#FAF6EF;font-weight:600;margin:0 0 24px;">${escapeHtml(cartLine)}</p>
        ${ctaButton(link, "Finish Booking →")}
      </div>`, `Still waiting for you: ${cartLine}.`);
  }
  return marketingEmailShell(`
    <div style="background:#12100e;border:1px solid rgba(232,168,46,0.35);border-radius:18px;padding:32px 24px;text-align:center;color:#FAF6EF;">
      ...
      <p style="font-size:14px;color:#B8A995;line-height:1.6;margin:0 0 20px;">Still waiting for you:</p>
      <p style="font-size:14px;color:#FAF6EF;font-weight:600;margin:0 0 24px;">${escapeHtml(cartLine)}</p>
      ${ctaButton(link, "Finish & Save 10% →")}
    </div>`, `10% off your saved order (${cartLine}) — today only.`);
}
