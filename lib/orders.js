 ...
 */
export function buildOrder({ paymentIntent, stripeSession, cartItems, specialInstructions, tip = 0 }) {
  ...
  return {
    ...
    // Financials (in dollars) — total is what the kitchen/dashboard should
    // treat as authoritative; it excludes the CC processing fee (that's
    // Stripe's cut, not restaurant revenue) but includes the tip.
    subtotal:           subtotal,
    tax:                tax,
