...
export async function saveOrder(order) {
  ...
  // 3rd paid order on a signed-in account auto-mints and emails a 10% off
  ...
  if (order.clerkUserId) {
    ...
    // so llen pins at 200 forever and 200 % LOYALTY_MILESTONE_EVERY === 0
    // would mean every single order past #200 mints a voucher (verified
    // against the old 5-order milestone: 15/15 orders after 200 did).
...
const LOYALTY_MILESTONE_EVERY = 3;
...
function loyaltyRewardEmailHtml({ customerName, count, token }) {
  ...
  <div style="font-family:'Inter',sans-serif;background:#080706;padding:32px 16px;color:#FAF6EF;">
    <div style="max-width:480px;margin:0 auto;background:#12100e;border:1px solid rgba(232,168,46,0.35);border-radius:18px;padding:32px 24px;text-align:center;">
      ...
      <p style="font-size:11px;color:#8A7560;margin:8px 0 0;"><a href="${BASE_URL}/rewards" style="color:#B8A995;">How the Rani Royal Club works →</a></p>
    </div>
  </div>`;
}
