 ...
 * Includes orders from yesterday if they are still active / unfulfilled so
 * no active ticket is ever dropped across date boundaries.
 */
export async function getOrdersByDate(date) {
  ...
  // Also query yesterday's index to catch unfulfilled/active orders crossing midnight
  ...
  const allIds = Array.from(new Set([...ids, ...prevIds]));
