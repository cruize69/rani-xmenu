...
export async function saveOrder(order) {
  ...
  if (order.status === ORDER_STATUS.SCHEDULED && order.scheduledAtMs) {
    await kv.zadd("scheduled-orders", { score: order.scheduledAtMs, member: order.id });
  }
  ...
  if (order.clerkUserId) {
    try {
      const count = await kv.llen(`account-orders:${order.clerkUserId}`);
      if (count > 0 && count % LOYALTY_MILESTONE_EVERY === 0 && order.customerEmail) {
