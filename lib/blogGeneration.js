...
export async function runBlogGenerationPipeline({ photoUrls = [], calendarHint = null, calendarBlurb = null, sourceLabel = "unknown" }) {
  try {
    ...
    // The Gemini responseSchema's `required` only enforces key PRESENCE,
    // not non-empty/minimum length — a technically-valid response with an
    // empty or near-empty body would sail through that check and open a
    // real PR against the live marketing repo with a garbage post. This is
    // the actual content-sanity gate.
    const MIN_BODY_LENGTH = 200;
    if (!draft.title?.trim() || !draft.slug?.trim() || !draft.body?.trim() || draft.body.trim().length < MIN_BODY_LENGTH) {
      console.error(`Blog pipeline (${sourceLabel}): draft failed sanity check (title="${draft.title}", slug="${draft.slug}", body length=${draft.body?.length ?? 0}), aborting before PR`);
      return null;
    }
