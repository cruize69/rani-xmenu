...
const GITHUB_OWNER = "riyadhjuwel"; // TODO: confirm this is the correct org/user for ranimahal-marketing
const GITHUB_REPO = "ranimahal-marketing";
...
// call entirely. Photo-driven runs with no hint get classified into the
// fixed taxonomy from the captioning output, keeping topic selection
...
export async function resolveTopic({ calendarHint, captionResult }) {
  ...
  if (!captionResult?.photos?.length) return { pillar: "dish-spotlight", topicSlug: "general" };
  ...
  const result = await geminiGenerate({ parts: [{ text: promptText }], responseSchema: schema });
  return result || { pillar: "dish-spotlight", topicSlug: "general" };
}
...
Topic angle: ${topic.pillar} — ${topic.topicSlug}
