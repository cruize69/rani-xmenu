...
// bookings (that's what MIN_LEAD_TIME_MIN handles): a customer ordering at
// 2pm for tonight's 5:00 dinner opening was landing on exactly 5:00 with no
// buffer, which is what actually happened in production.
 ...
 * "Lunch". The first slot of every window is pushed out by KITCHEN_RAMP_MIN
 * past opening; if the window is later today, it's pushed out further still
 * to at least MIN_LEAD_TIME_MIN from now (whichever constraint is later
 * wins), rounded up to the next quarter hour.
 */
export function getTimeSlots(dateStr, window, now = new Date()) {
  ...
  let startMin = openMin + KITCHEN_RAMP_MIN;
  if (dateStr === todayStr) {
...
export function isWithinServiceWindow(dateStr, hhmm, now = new Date()) {
  ...
  // Mirrors getTimeSlots' KITCHEN_RAMP_MIN buffer — this is the real
  // security boundary, so a request can't bypass the picker's ramp-up
  // buffer by hand-crafting a scheduledFor of exactly opening time.
  return dayHours.services.some(s => minutes >= toMinutes(s.opens) + KITCHEN_RAMP_MIN && minutes < toMinutes(s.closes));
}
