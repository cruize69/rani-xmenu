...
export function getTimeSlots(dateStr, window, now = new Date(), orderMode = "pickup") {
  ...
  const slots = [];
  // Delivery stops one interval short of closing — an order placed right at
  // close gives the kitchen no runway before it also has to go out the
  // door. Pickup doesn't need that same runway (no drive time on top), so
  // its last bookable slot is the service's actual closing time (e.g. the
  // 2:30 PM lunch close) rather than 15 minutes earlier.
  const lastSlotMin = orderMode === "delivery" ? closeMin - SLOT_INTERVAL_MIN - 1 : closeMin;
  for (let m = startMin; m <= lastSlotMin; m += SLOT_INTERVAL_MIN) {
