...
export const SECTIONS = [
  // Rendered specially by RaniMahal.jsx (feast cards, not the standard
  // item grid) — subsections stays empty on purpose; FEASTS (lib/feasts.js)
  // is the real data source for this section's content. First in the list
  // deliberately: the whole point is to intercept decision fatigue before
  // a customer even reaches the individual-item grid, so it has to be the
  // first thing shown, not just another tab alongside Appetizers.
  // id/eyebrow/title deliberately say "Family Meals", not "Feasts" — see
  // lib/feasts.js's header comment: "Feast" is already the catering menu's
  // "Rani Feast" tier, and reusing it here was cannibalizing both in
  // search and confusing customers about which product they'd landed on.
  { id:"family-meals", eyebrow:"Feeding a group?", title:"Family Meals", note:"", subsections:[] },
...
// Every order's `items` array (see lib/orders.js's buildOrder) carries each
// line's original baseId — this is the one place that decides "was this
// order (at least partly) a catering order," so every lifecycle-marketing
// touchpoint that needs to special-case catering (win-back copy, reorder-
// voucher wording, cross-sell targeting, etc.) asks this instead of
// re-deriving its own catering-detection logic.
export function isCateringOrder(order) {
  return (order?.items || []).some(i => CATERING_ITEM_IDS.has(i.baseId));
}
