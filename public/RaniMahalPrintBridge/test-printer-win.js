...
const sampleOrder = {
  ...
  customerPhone: "9144411103",
  items: [
    { name: "RAITA", qty: 1, price: 4.50 },
    { name: "BIRIYANI MEDLEY", qty: 1, price: 27.95 },
    { name: "PESHWARI NAAN", qty: 1, price: 6.20, spice: "mild" },
    { name: "MIXED APPETIZERS", qty: 1, price: 9.95 },
    { name: "NIMBU PANI", qty: 1, price: 6.00 },
  ],
  subtotal: 54.60,
  tax: 4.57,
  total: 72.49,
  specialInstructions: "PACK EXTRA CHUTNEY ON SIDE",
};
...
function printJob(textContent) {
  return new Promise((resolve, reject) => {
    const tmpFile = path.join(os.tmpdir(), "rm_test_job.txt");
    ...
    exec(`powershell -NoProfile -ExecutionPolicy Bypass -Command "${psScript}"`, { timeout: 15000 }, (err) => {
      if (err) reject(err);
      else resolve();
    });
  });
